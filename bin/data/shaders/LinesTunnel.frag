...
void main()
{
    ...
    vec2 st = (2. * gl_FragCoord.xy / iResolution.xy - 1.)
    ...
    float offset = time/freq;
	   ...
	   vec2  t = vec2(atan(st.x, st.y) / 3.1416, 1. / length(st*2.));
  ...
  //  rings += ring(stDot, mod(time+100., 2.), 0.5, 0.02);
    vec3 color = vec3(noise*(blackDot)*(1.-rings));
